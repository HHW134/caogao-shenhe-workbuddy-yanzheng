...
| 模块标识 | 名称 | 覆盖内容 | 规则来源 |
| --- | --- | --- | --- |
| `chapters` | 术语与引用联动审核 | 术语条目双语/大小写/引导语/条款约束/正文使用；引用条目编号、排序、层级、年份、占位符；**两章节联动**（术语「来源」标准不得列入规范性引用文件） | `terminology_rules.json` + `form_validation_term_ref_rules.json` |
...
| 序号  | 审核要点                 | 级别   | 问题                                 |
+-------+--------------------------+--------+--------------------------------------+
| 1     | 规范性引用文件-排序      | 错误   | 规范性引用文件排序错误（同类型内未…  |
| 11    | 规范性引用文件-来源联动  | 错误   | 该标准出现在术语和定义章节中，不应…  |
```
